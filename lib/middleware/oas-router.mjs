    ...
    if (resultType && resultType.essence === "application/json") {
      ...
      if (err === false) {
        ...
        if (config.strict === true) {
          ...
          config.logger.warn(JSON.stringify(msg) + JSON.stringify(validator.getLastErrors()));
          if (content[0].substr(0, 46) === '{"message":"This is the mockup controller for ') {
            config.logger.warn("The used controller might not have been implemented");
          }
          oldSend.apply(res, content);
        }
      } else {
...
function existsController(locationOfControllers, controllerName) {
  try {
    ...
    config.logger.info("The controller " + controllerName + " doesn't exist at " + locationOfControllers);
    return false;
  }
}
...
//# sourceMappingURL=oas-router.mjs.map
...
const schemaV3 = jsyaml.safeLoad(fs.readFileSync(join(__dirname, "./schemas/openapi-3.0.yaml"), "utf8"));
...
function checkOperationId(load, pathName, methodName, methodSection) {
  ...
  if (opId == undefined) {
    ...
    config.logger.debug("      There is no operationId for " + methodName.toUpperCase() + " - " + pathName + " -> generated: " + opId);
  }
  if (load[opId] == undefined) {
    ...
    config.logger.debug("      Controller for " + methodName.toUpperCase() + " - " + pathName + ": OK");
  }
}
...
function checkControllers(pathName, methodName, methodSection, controllersLocation) {
  ...
  if (methodSection[router_property] != undefined) {
    ...
    config.logger.debug("    OAS-doc has " + router_property + " property " + controller);
    try {
      load = require(join(controllersLocation, utils.generateName(controller, undefined)));
    ...
    config.logger.debug("    Spec-file does not have router property -> try generic controller name: " + controller);
    try {
      load = require(join(controllersLocation, controller));
      checkOperationId(load, pathName, methodName, methodSection);
    } catch (err) {
      config.logger.debug("    Controller with generic controller name wasn't found either -> try Default one");
      try {
    ...
    Object.keys(config.securityFile).forEach(secName => {
      if (typeof config.securityFile[secName] === "string" && isJWTScheme(specDoc.components.securitySchemes[secName])) {
        if (config.securityFile[secName].substr(0, 4) === "http") {
          ...
          config.securityFile[secName] = require(join(process.cwd(), config.securityFile[secName]));
        }
      }
    });
    Object.keys(config.grantsFile).forEach(secName => {
      if (typeof config.grantsFile[secName] === "string" && isJWTScheme(specDoc.components.securitySchemes[secName])) {
        if (config.grantsFile[secName].substr(0, 4) === "http") {
          ...
          config.grantsFile[secName] = extendGrants(specDoc, require(join(process.cwd(), config.grantsFile[secName])));
        }
      } else {
...
function registerPaths(specDoc, app) {
  ...
  if (specDoc.servers) {
    ...
    if (!localServer) {
      config.logger.info("No localhost or relative server found in spec file, added for testing in Swagger UI");
    ...
    config.logger.info("No servers found in spec file, added relative server for testing in Swagger UI");
    specDoc.servers = [{
      url: "/"
    }];
  }
  ...
  for (var path in paths) {
    for (var method in paths[path]) {
      if (allowedMethods.includes(method)) {
        ...
        config.logger.debug("Register: " + method.toUpperCase() + " - " + expressPath);
  ...
  if (config.docs && config.docs.apiDocs) {
    ...
    if (config.docs.swaggerUi) {
      var uiHtml = fs.readFileSync(join(__dirname, "../swagger-ui/index.html"), "utf8");
      uiHtml = uiHtml.replace(/url: "[^"]*"/, 'url: "' + config.docs.apiDocsPrefix + config.docs.apiDocs + '"');
      fs.writeFileSync(join(__dirname, "../swagger-ui/index.html"), uiHtml, "utf8");
      if (!config.docs.swaggerUiPrefix) {
        config.docs.swaggerUiPrefix = "";
      }
      app.use(config.docs.swaggerUiPrefix + config.docs.swaggerUi, express.static(join(__dirname, "../swagger-ui")));
    }
  }

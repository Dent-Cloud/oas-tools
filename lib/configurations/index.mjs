...
function createNewLogger() {
  ...
  const transports = [new winston.transports.Console({
    ...
    json: false,
    format: winston.format.combine(winston.format.colorize(), winston.format.timestamp(), winston.format.splat(), customFormat)
  })];
  if (config.logfile != undefined) {
    transports.push(new winston.transports.File({
      ...
      maxsize: 5242880,
      //5MB
      format: winston.format.combine(winston.format.timestamp(), winston.format.splat(), customFormat)
    }));
  }

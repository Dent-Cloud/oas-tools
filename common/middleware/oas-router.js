...
var _lodashCompat = _interopRequireDefault(require("lodash-compat"));
...
function checkResponse(req, res, oldSend, oasDoc, method, requestedSpecPath, content) {
  ...
  _configurations.config.logger.debug("Processing at checkResponse:");
  _configurations.config.logger.debug("  -code: " + code);
  _configurations.config.logger.debug("  -oasDoc: " + JSON.stringify(oasDoc));
  _configurations.config.logger.debug("  -method: " + method);
  _configurations.config.logger.debug("  -requestedSpecPath: " + requestedSpecPath);
  _configurations.config.logger.debug("  -data: " + JSON.stringify(data));
  ...
  if (res.get("content-type") === undefined) {
    ...
    explicitType = new _whatwgMimetype.default(res.get("content-type"));
  }
  if (responseCodeSection === undefined) {
    ...
    if (_configurations.config.strict === true) {
      ...
      _configurations.config.logger.warn(JSON.stringify(msg));
      oldSend.apply(res, content);
    }
    ...
    acceptTypes.forEach(acceptType => {
      var mimeAccept = new _whatwgMimetype.default(acceptType);
      Object.keys(responseCodeSection.content).forEach(contentType => {
        if (!resultType) {
          var firstMatch, secondMatch;
          var mimeContent = new _whatwgMimetype.default(contentType);
          if (explicitType) {
    ...
    if (!resultType && acceptTypes.length === 0) {
      resultType = new _whatwgMimetype.default("application/json");
    ...
    if (resultType && resultType.essence === "application/json") {
      ...
      var validSchema = cloneDeep(responseCodeSection.content["application/json"].schema);
      utils.fixNullable(validSchema);
      content[0] = JSON.stringify(content[0]);
      _configurations.config.logger.debug("Schema to use for validation: " + JSON.stringify(validSchema));
...
function existsController(locationOfControllers, controllerName) {
  try {
    require(_path.default.join(locationOfControllers, controllerName));
    return true;
  } catch (err) {
    _configurations.config.logger.info("The controller " + controllerName + " doesn't exist at " + locationOfControllers);
    return false;
  }
}
...
var _default = controllers => {
  return function OASRouter(req, res, next) {
    ...
    if (oasDoc.paths[requestedSpecPath].hasOwnProperty("x-swagger-router-controller") && oasDoc.paths[requestedSpecPath][method].hasOwnProperty("x-swagger-router-controller") === false) {
      oasDoc.paths[requestedSpecPath][method] = oasDoc.paths[requestedSpecPath]["x-swagger-router-controller"];
    }
    if (oasDoc.paths[requestedSpecPath].hasOwnProperty("x-router-controller") && oasDoc.paths[requestedSpecPath][method].hasOwnProperty("x-router-controller") === false) {
      oasDoc.paths[requestedSpecPath][method] = oasDoc.paths[requestedSpecPath]["x-router-controller"];
    }
    // end pgillis

    if (oasDoc.paths[requestedSpecPath][method].hasOwnProperty("x-swagger-router-controller")) {
    ...
    } else if (oasDoc.paths[requestedSpecPath][method].hasOwnProperty("x-router-controller")) {
      //oasDoc file has router_property: use the controller specified there
      controllerName = oasDoc.paths[requestedSpecPath][method]["x-router-controller"];
    } else if (existsController(controllers, utils.generateName(requestedSpecPath, "controller"))) {
    ...
    var controller = require(_path.default.join(controllers, controllerName));
    ...
    res.send = function send() {
      ...
      checkResponse(req, res, oldSend, oasDoc, method, requestedSpecPath,
...
//# sourceMappingURL=oas-router.js.map
...
function checkBody(req, requestBody) {
  ...
  if (requestBody.required && emptyBody) {
    ...
    _configurations.config.logger.info("Valid parameter on request");
  }
  return undefined;
}
function getRequestProperty(req, location, name) {
  ...
  if (location === "headers") {
    return requestLocation[Object.keys(requestLocation).find(key => key.toLowerCase() === name.toLowerCase())];
  }
...
function checkParameter(req, params) {
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    const nullable = currentParameter.nullable === undefined ? false : Boolean(currentParameter.nullable);
    const required = currentParameter.required === undefined ? false : Boolean(currentParameter.required);
    ...
    if (requestParameter === undefined) {
      ...
      if (nullable === false) {
        errorMessages.push({
          message: "The parameter " + name + " in " + location + " cannot be null. "
        });
      }
    } else {
      // In case the parameter is indeed present, check type. In the case of array, check also type of its items!
      const value = convertValue(requestParameter, schema, getParameterType(currentParameter), currentParameter);
      const err = validator.validate(value, schema);
      if (err == false) {
        if (err.code == "UNKNOWN_FORMAT") {
          var registeredFormats = _zSchema.default.getRegisteredFormats();
          _configurations.config.logger.error("UNKNOWN_FORMAT error - Registered Formats: ");
          _configurations.config.logger.error(registeredFormats);
        }
        errorMessages.push({
...
function checkRequestData(oasDoc, requestedSpecPath, method, res, req, next) {
  ...
  if (paths[requestedSpecPath][method].hasOwnProperty("requestBody")) {
    const message = checkBody(req, paths[requestedSpecPath][method].requestBody);
  ...
  if (keepGoing == false && _configurations.config.strict == true) {
    if (_configurations.config.customErrorHandling) {
...
function convertValue(value, optionalSchema, optionalType) {
  ...
  const type = optionalType === undefined ? getParameterType(schema) : optionalType;
  switch (type) {
    case "array":
...
var _default = oasDoc => {
  return function OASValidator(req, res, next) {
    ...
    _configurations.config.logger.info("Requested method-url pair: " + method + " - " + req.url);
    var requestedSpecPath = _configurations.config.pathsDict[removeBasePath(req.route.path)];
    ...
    _configurations.config.logger.debug("OASValidator  -res.locals.requestedSpecPath: " + res.locals.requestedSpecPath);
    checkRequestData(oasDoc, requestedSpecPath, method, res, req, next);
  };
};
exports.default = _default;
//# sourceMappingURL=oas-validator.js.map